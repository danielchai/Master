	...
	profile:
	   	...
	   	if(profiled == 0 )
		{
			...
			do{
				...
				if((sample_indicator!= 0)&&(sample == 0))
				{	
					//If it is the sample time but the time has passed 10 minutes then do not sample, because the SDL already off. This is used
					//to avoid the auto sample when the user recycle the power
					if(curr_time[4]>10)goto profile;
					if((curr_time[4]>=4) && (curr_time[4]<=10))
					{			
						login("start sampling");
						enable(0);
						initbus(1);
						sleep(1);
						ctx = modbusconnection(ctx);
...
position1:
		...
		sdl_upload:
			...
			if(sdl == NULL)
			{
				if(re_send < 3){re_send++;goto sdl_upload;}
				else  
				{	
					login("Failed in upload data connection");
					setsdl(30000,-100000);
				}	
			}
			else profile_save_data(sdl);
			modbus_close(sdl);	
			goto profile;
		}
		goto profile;
