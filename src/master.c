...
int process_expression(int argsize, int arg1, int arg2, int arg3)
{
...
int process_pass_through_check()
{
	PyObject *arg, *mymod, *strfunc;
	PyObject *pystatus;
	int status;
	Py_Initialize();
	mymod = PyImport_ImportModule("python_database");
	strfunc = PyObject_GetAttrString(mymod,"db_select_par");
	arg = Py_BuildValue("(i)",5);
	pystatus = PyEval_CallObject(strfunc,arg);
	PyArg_ParseTuple(pystatus,"i",&status);
	//printf("the database is %d\n",status);
	Py_Finalize();	
	return status;
}
double process_check_last_position(int i)
{
	PyObject *arg, *mymod, *strfunc;
	PyObject *pystatus;
	double status;
	Py_Initialize();
	mymod = PyImport_ImportModule("python_database");
	strfunc = PyObject_GetAttrString(mymod,"db_select_lr");
	arg = Py_BuildValue("(i)",9);
	pystatus = PyEval_CallObject(strfunc,arg);
	PyArg_ParseTuple(pystatus,"d",&status);
	printf("the database is %f\n",status);
	Py_Finalize();
	return status;
}

int process_go_up(int slave_id)
{   
    PyObject *arg, *mymod, *strfunc;
    PyObject *pystatus;
    int status;
    ...
    strfunc = PyObject_GetAttrString(mymod,"go_up");
...
int process_go_down(int slave_id)
{
	PyObject *arg, *mymod, *strfunc;
	PyObject *pystatus;
	int status;
	Py_Initialize();
	mymod = PyImport_ImportModule("python_motor");
	strfunc = PyObject_GetAttrString(mymod,"go_down");
	arg = Py_BuildValue("(i)",slave_id);
	pystatus = PyEval_CallObject(strfunc,arg);
	status = PyObject_IsTrue(pystatus);
	//printf("status is %d\n",status);
...
int process_go_home(int slave_id)
{
     PyObject *arg, *mymod, *strfunc;
     PyObject *pystatus;
     int status;
     Py_Initialize();
     mymod = PyImport_ImportModule("python_motor");
     strfunc = PyObject_GetAttrString(mymod,"go_home");
     arg = Py_BuildValue("(i)",slave_id);
     pystatus = PyEval_CallObject(strfunc,arg);
     status = PyObject_IsTrue(pystatus);
     //printf("status is %d\n",status);
...
int process_check(int slave_id)
{
	PyObject *arg, *mymod, *strfunc;
	PyObject *pystatus;
	int status = -1;
	Py_Initialize();
	mymod = PyImport_ImportModule("python_motor");
	strfunc = PyObject_GetAttrString(mymod,"motor_status");
	arg = Py_BuildValue("(i)",slave_id);
	pystatus = PyEval_CallObject(strfunc,arg);
	PyArg_Parse(pystatus,"i",&status);
	//printf("status is %d\n",status);
	Py_Finalize();
	return status;
}
int process_stop(int slave_id)
{
	PyObject *arg, *mymod, *strfunc;
	PyObject *pystatus;
	int status = -1;
	Py_Initialize();
    mymod = PyImport_ImportModule("python_motor");
    strfunc = PyObject_GetAttrString(mymod,"stop");
    arg = Py_BuildValue("(i)",slave_id);
    pystatus = PyEval_CallObject(strfunc,arg);
	status = PyObject_IsTrue(pystatus);
    //printf("status is %d\n",status);
...
float process_position(int step)
{
	PyObject *arg, *mymod, *strfunc;
	PyObject *pystatus;
	float position = -1;
	Py_Initialize();
    mymod = PyImport_ImportModule("python_motor");
    strfunc = PyObject_GetAttrString(mymod,"get_position");
    arg = Py_BuildValue("(i)",step);
    pystatus = PyEval_CallObject(strfunc,arg);
    PyArg_Parse(pystatus,"f",&position);
    //printf("position is %f\n",position);
    Py_Finalize();
	return position;
}
int process_syncclock()
{
	...
	Py_Initialize();
	mymod = PyImport_ImportModule("python_send");
	strfunc = PyObject_GetAttrString(mymod,"exe_cmd_sync_clock");
	//printf("sys1\n");
	pystatus = PyObject_CallObject(strfunc,NULL);
	//printf("sysnc2\n");
	status = PyObject_IsTrue(pystatus);
	//printf("%d",status);
	Py_Finalize();
...
int process_read_home_switch(int slave_id)
{
	PyObject *arg, *mymod, *strfunc;
	PyObject *pystatus;
	int status = -1;
	Py_Initialize();
	mymod = PyImport_ImportModule("python_motor");
	strfunc = PyObject_GetAttrString(mymod,"read_home_switch");
	arg = Py_BuildValue("(i)",slave_id);
	pystatus = PyEval_CallObject(strfunc,arg);
	PyArg_Parse(pystatus,"i",&status);
	//printf("%d",status);
...
************************************************************************/
	...
	start_profile:
		...
		if ( profiled == 0 ) 
		{
			...
			if( process_read_home_switch(1) != 1)
			{
...
************************************************************************/
	...
	*/
		...
		printf("The first %f", process_check_last_position(0));
		//printf("The second %d", process_expression(3,1,3,0));
		//printf("The third %d", process_read_home_switch(1));
    }
